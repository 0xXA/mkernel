...
function check_shell {
	if [[ -z "${BASH}" ]] || [[ ${BASH_VERSINFO} < 4 ]]; then
		pr_err "This script can only be executed in bash 4+"
	fi
}

function get_manifest {
	if [[ ! -e "${MKERNEL_PROP_FILE}" ]]; then
...
function build_kernel {
	...
	if [[ -z "${MKERNEL_FORCE_REBUILD}" ]]; then
		[[ -e ${MKERNEL_PROP_DIR}/${MKERNEL_TARGET_CONFIG/_*} ]] && {
			pr_info "Use '${MKERNEL_PROGRAM} -f' to force rebuild kernel."
			pr_err "A build directory '${MKERNEL_TARGET_CONFIG/_*}' already exists."
		}
	else
		rm -rf "${MKERNEL_PROP_DIR}/${MKERNEL_TARGET_CONFIG/_*}*"
	fi
	...
	[[ -z "${MKERNEL_BUILD_DIR}" ]] && {
		MKERNEL_BUILD_DIR="${MKERNEL_TOP_DIR}/${MKERNEL_TARGET_CONFIG/_*}"
	        [[ ! -e "${MKERNEL_BUILD_DIR}" ]] && \
	...
	if [[ -n "${MKERNEL_CREATE_LOGFILE}" ]];then
		make -j${MKERNEL_USE_CORES:-$(nproc)} O=${MKERNEL_BUILD_DIR} ${MKERNEL_TARGET_CONFIG:?} &>> ${MKERNEL_TARGET_CONFIG}.log
		make -j${MKERNEL_USE_CORES:-$(nproc)} O=${MKERNEL_BUILD_DIR} &>> ${MKERNEL_TARGET_CONFIG/_*}_kernel.log
	else
		make -j${MKERNEL_USE_CORES:-$(nproc)} O=${MKERNEL_BUILD_DIR} ${MKERNEL_TARGET_CONFIG:?}
...
STDOUT
    ...
    show_version)
cat << STDOUT
                  ${MKERNEL_PROGRAM} ${MKERNEL_VERSION}
